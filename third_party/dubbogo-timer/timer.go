...
type TimerWheel struct {
	...
	// LOCAL PATCH: explicit signal so the worker goroutine can exit its
	// select promptly when Stop()/Close() is called. See PATCHES.md.
	quit chan struct{}
}
...
func NewTimerWheel() *TimerWheel {
	...
	LOOP:
		for {
			...
			select {
			// LOCAL PATCH: observe quit signal so a parked select can exit
			// when Stop()/Close() is called. See PATCHES.md.
			case <-w.quit:
				break LOOP
...
func (w *TimerWheel) Stop() {
	w.once.Do(func() {
		...
		// LOCAL PATCH: wake up the worker goroutine that may be parked in
		// its main-loop select. ticker.Stop() does not close ticker.C, and
		// closing timerQ races with AddTimer (the upstream comment above is
		// "// close(w.timerQ) // to defend data race warning"), so we use a
		// dedicated quit channel. See PATCHES.md.
		close(w.quit)
